...
interface CSMTicketInboxProps {
  viewMode?: TicketViewMode;
  assignedStudentIds?: { ids: Set<string>; emails: Set<string> };
}

export function CSMTicketInbox({ viewMode = 'csm', assignedStudentIds }: CSMTicketInboxProps) {
  const [myTicketsOnly, setMyTicketsOnly] = useState(!!assignedStudentIds);
  ...
  const filtered = tickets.filter((t: any) => {
    ...
    if (myTicketsOnly && assignedStudentIds) {
      const byId = t.submitter_user_id && assignedStudentIds.ids.has(t.submitter_user_id);
      const byEmail = t.submitter_email && assignedStudentIds.emails.has(t.submitter_email.toLowerCase());
        ...
        <div className="inline-flex items-center rounded-lg border p-1 bg-muted/60">
          <button
            ...
            className={`px-5 py-2 rounded-md text-sm font-medium transition-colors ${
              myTicketsOnly ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
            }`}
          >
            My Tickets ({tickets.filter((t: any) => {
              const byId = t.submitter_user_id && assignedStudentIds.ids.has(t.submitter_user_id);
              const byEmail = t.submitter_email && assignedStudentIds.emails.has(t.submitter_email.toLowerCase());
              return byId || byEmail;
            }).length})
