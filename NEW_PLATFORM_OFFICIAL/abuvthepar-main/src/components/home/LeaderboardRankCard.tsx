...
export function LeaderboardRankCard() {
  ...
  const { data: previewData, isLoading } = useQuery({
    ...
    select: (data) => {
      if (!data || !('entries' in data)) {
        // Data comes from LeaderboardSidePanel's shape (array) — transform it
        const arr = data as any as PreviewEntry[];
        if (!arr || !Array.isArray(arr)) return { entries: [], myEntry: null, showSeparator: false };
        const myEntry = arr.find((e) => e.user_id === user?.id) ?? null;
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center gap-2.5 px-3 py-2.5 bg-gradient-to-r from-primary/35 via-primary/15 to-transparent">
