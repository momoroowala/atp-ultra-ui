  ...
  useEffect(() => {
    const fetchAndEmbedVideo = async () => {
      try {
        ...
        // Extract and execute the script after a short delay to ensure div is rendered
        const scriptTag = doc.querySelector('script');
        if (scriptTag && !scriptLoadedRef.current) {
          console.log('[Vidalytics] Found script, will execute after DOM update');
          
          // Wait for next tick to ensure the div is in the DOM
          setTimeout(() => {
            console.log('[Vidalytics] Executing script');
            const script = document.createElement('script');
            script.type = 'text/javascript';
            script.text = scriptTag.textContent || '';
            document.head.appendChild(script);
            scriptLoadedRef.current = true;
            console.log('[Vidalytics] Script appended and executed');
          }, 100);
        }
