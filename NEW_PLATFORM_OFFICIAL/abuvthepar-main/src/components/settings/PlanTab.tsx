...
export const PlanTab = () => {
  ...
  const togglePhaseActive = useMutation({
    ...
    onSuccess: async (_, variables) => {
      await queryClient.invalidateQueries({ queryKey: ['phases', selectedCourseId], refetchType: 'active' });
      await queryClient.invalidateQueries({ queryKey: ['phase-tasks-bulk'], refetchType: 'active' });
      toast({ title: variables.is_active ? 'Phase activated successfully' : 'Phase deactivated successfully' });
    },
  });
  ...
  const deletePhase = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['phases', selectedCourseId], refetchType: 'active' });
      await queryClient.invalidateQueries({ queryKey: ['phase-tasks-bulk'], refetchType: 'active' });
      toast({ title: 'Phase permanently deleted' });
    },
    onError: (error: any) => {
      toast({ 
  ...
  const reorderPhases = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['phases', selectedCourseId], refetchType: 'active' });
      await queryClient.invalidateQueries({ queryKey: ['phase-tasks-bulk'], refetchType: 'active' });
      toast({ title: 'Phase order updated' });
    },
    onError: (error: any) => {
      toast({
        title: 'Failed to update phase order',
        description: error.message,
        variant: 'destructive'
      });
    },
