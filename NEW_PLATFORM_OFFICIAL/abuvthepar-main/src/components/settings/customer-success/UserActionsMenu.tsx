  ...
  const handleConfirmOnboarding = async () => {
    ...
    try {
      ...
      if (selectedOnboardingStatus === 'completed') {
        const now = new Date();
        updateData.onboarding_date = now.toISOString();
        const offboarding = new Date(now);
        offboarding.setMonth(offboarding.getMonth() + 6);
        updateData.offboarding_date = offboarding.toISOString().slice(0, 10);
      }
      ...
      // Remove from missed onboarding queue when completed
      if (selectedOnboardingStatus === 'completed') {
        await supabase
          .from('missed_onboarding_events')
          .update({ rescheduled: true })
          .eq('user_id', userId);
        queryClient.invalidateQueries({ queryKey: ['missed-onboarding-events'] });
      }

      // Fire GHL webhook for missed onboarding (non-blocking)
      if (selectedOnboardingStatus === 'missed') {
        try {
          const { error: webhookError } = await supabase.functions.invoke(
            'send-missed-onboarding-webhook',
            {
              body: {
                email: userEmail,
                ...
                phone: userPhone,
                user_id: userId,
                action: 'missed_onboarding',
                triggered_at: new Date().toISOString(),
              },
            }
          );
          if (webhookError) {
            console.error('Missed onboarding webhook error:', webhookError);
            toast.warning('Onboarding marked as missed, but the SMS sequence failed to trigger. Please notify the team.');
          }
        } catch (whErr) {
          console.error('Missed onboarding webhook exception:', whErr);
          toast.warning('Onboarding marked as missed, but the SMS sequence failed to trigger.');
        }
      }
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={selectedTierId} onValueChange={setSelectedTierId}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {tiers?.filter(t => isCSM ? t.tier_key !== 'staff' : true).map((tier) => (
      ...
      <Dialog open={editUserOpen} onOpenChange={setEditUserOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Update the user's name, email, and phone number.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
            ...
            <Button disabled={loading} onClick={async () => {
              ...
              try {
                const { error: nameError } = await supabase
                  .from('user_profiles')
                  .update({ first_name: editFirstName.trim(), last_name: editLastName.trim(), phone: editPhone.trim() || null })
                  .eq('id', userId);
                if (nameError) throw nameError;

                if (editEmail.trim() !== userEmail) {
                  const { data, error } = await supabase.functions.invoke('update-user-email' as any, {
