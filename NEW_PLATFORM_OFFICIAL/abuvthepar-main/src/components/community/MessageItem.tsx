import { memo, useState, useRef, useMemo, useEffect, useCallback, KeyboardEvent } from 'react';
...
interface MessageItemProps {
  ...
  // Receives the full message so callers can keep a stable reference instead of
  // closing over `message` and breaking React.memo.
...
const MessageItemInner = ({ message, isConsecutive, onReply, isReply = false, onPin, onUnpin, onDeleteMessage, onEditMessage, isDM = false, isReadOnly = false }: MessageItemProps) => {
  // Stable handler that forwards the current message to the parent's callback.
        ...
        <div className="flex-1 min-w-0">
          {!isConsecutive && (
            <div className="flex items-baseline gap-2 mb-1">
              <UserProfilePopover userId={message.sender_id}>
                <span className={`font-semibold text-foreground text-sm hover:underline`}>{name}</span>
              </UserProfilePopover>
          ...
          {isEditing ? (
            <div className="flex flex-col gap-1">
              <textarea
                ...
                className="w-full bg-muted border border-border rounded-md px-3 py-2 text-sm outline-none focus:border-primary resize-none"
              />
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <span>Enter to save · Escape to cancel · Shift+Enter for new line</span>
                <div className="flex gap-1 ml-auto">
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => { setIsEditing(false); setEditContent(message.content); }}>
                    <X className="h-3 w-3" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-primary" onClick={handleEditSave}>
            ...
            <div className={`prose prose-sm dark:prose-invert max-w-none break-words whitespace-pre-wrap ${isReply ? 'text-sm' : ''}`}>
