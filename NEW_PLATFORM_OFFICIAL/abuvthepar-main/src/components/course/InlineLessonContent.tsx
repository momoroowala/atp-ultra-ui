...
import { Edit2, RotateCcw, Loader2, Check, X, Clock, Award, BookOpen, Pencil } from 'lucide-react';
...
interface InlineLessonContentProps {
  ...
  onProgress80?: () => void;
  /** When true, shows edit buttons for description and readout */
  isAdmin?: boolean;
}
...
export function InlineLessonContent({
  ...
  const handleSaveDescription = async () => {
    if (!task) return;
    setIsSaving(true);
    const { error } = await supabase.from('tasks' as any).update({ description: descriptionDraft }).eq('id', task.id);
    setIsSaving(false);
    if (error) { toast.error('Failed to save description'); return; }
    toast.success('Description updated');
    setEditingDescription(false);
    invalidateQueries();
  };

  const handleSaveReadout = async () => {
    if (!task) return;
    const rs = (task.sections || []).find(s => s.section_type === 'readout');
    if (!rs) return;
    setIsSaving(true);
    const { error } = await supabase.from('discipline_task_sections').update({ data: { content_html: readoutDraft } }).eq('id', rs.id);
    setIsSaving(false);
    if (error) { toast.error('Failed to save readout'); return; }
    toast.success('Readout updated');
    ...
    <div className="flex-1 flex flex-col overflow-y-auto">
      ...
      <div className="flex-1 p-6 pb-32">
        ...
        {(task.description || isAdmin) && (
          <div className="mb-6 group/desc relative">
              ...
              {editingDescription ? (
                <div className="flex-1 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => setEditingDescription(false)} disabled={isSaving}>
                      Cancel
                    </Button>
                  </div>
                </div>
                ...
                <>
                  <p className="text-foreground leading-relaxed flex-1">{task.description || (isAdmin ? <span className="text-muted-foreground italic">No description yet</span> : null)}</p>
                  {isAdmin && (
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 opacity-0 group-hover/desc:opacity-100 transition-opacity shrink-0"
        ...
        {readoutSection?.data && typeof readoutSection.data === 'object' && 'content_html' in readoutSection.data && (
          <div className="mb-6 group/readout relative">
            <div className="flex items-start gap-2 mb-4">
              <h3 className="text-lg font-semibold text-foreground flex-1">What You'll Learn:</h3>
              {isAdmin && !editingReadout && (
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-7 w-7 opacity-0 group-hover/readout:opacity-100 transition-opacity shrink-0"
                  onClick={() => { setReadoutDraft(String(readoutSection.data.content_html)); setEditingReadout(true); }}
                >
                  <Pencil className="h-3.5 w-3.5" />
                </Button>
              )}
            ...
            {editingReadout ? (
              <div className="space-y-2">
                <Suspense fallback={<Loader2 className="h-5 w-5 animate-spin text-primary" />}>
                  <ReactQuill theme="snow" value={readoutDraft} onChange={setReadoutDraft} className="bg-background rounded-md" />
                </Suspense>
                <div className="flex gap-2 mt-2">
                  <Button size="sm" onClick={handleSaveReadout} disabled={isSaving}>
                    {isSaving ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : null}
                    Save
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => setEditingReadout(false)} disabled={isSaving}>
                    Cancel
                  </Button>
                </div>
              </div>
