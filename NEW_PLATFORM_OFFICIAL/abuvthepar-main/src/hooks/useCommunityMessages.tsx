...
export const useCommunityMessages = (channelId?: string, dmConversationId?: string) => {
  ...
  const { data: cachedUserProfile } = useQuery({
    ...
    enabled: !!user?.id && !!(channelId || dmConversationId),
  ...
  useEffect(() => {
    ...
    const subscriptionKey = channelId || dmConversationId || '';
    ...
    // Scope realtime to the specific channel/DM at the Postgres replication layer
    // so unrelated messages never reach this client. The in-callback guards below
    // remain as a safety net in case the filter is ever missing.
    const realtimeFilter = channelId
  ...
  const sendMessage = useMutation({
    mutationFn: async (input: SendMessageInput) => {
      ...
      if (input.channel_id) {
        // Call the moderation edge function for channel messages
        ...
        if (moderationResult.flagged) {
          toast.warning('Your message is under review');
        }

        // Trigger push notifications only if not flagged
        if (!moderationResult.flagged) {
          supabase.functions.invoke('send-push-notification', {
            body: {
              ...
              messageId: moderationResult.message.id,
            },
          }).then(response => {
            if (response.error) {
        ...
        return { message: moderationResult.message, flagged: moderationResult.flagged, isReply: !!input.parent_message_id };
      } else {
...
export const useThreadReplies = (parentMessageId?: string) => {
  ...
  const repliesQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      // Batch-fetch all unique sender profiles in a single round trip instead
      // of one query per reply.
      const senderIds = Array.from(
        new Set((data || []).map((m) => m.sender_id).filter((id): id is string => !!id))
      );

      let senderById = new Map<string, {
        id: string;
        first_name: string | null;
        last_name: string | null;
        user_email: string | null;
      ...
      if (senderIds.length > 0) {
        const { data: senders } = await supabase
          ...
          .select('id, first_name, last_name, user_email, avatar_url')
          .in('id', senderIds);
        senderById = new Map((senders || []).map((s) => [s.id, s]));
      }

      const repliesWithSenders: CommunityMessage[] = (data || []).map((message) => ({
        ...message,
        attachments: (message.attachments || []) as unknown as MessageAttachment[],
