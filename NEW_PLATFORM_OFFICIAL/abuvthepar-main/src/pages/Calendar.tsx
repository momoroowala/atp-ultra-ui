...
import { isCallUpcomingInUserTZ, getDateInUserTimezone, getDateInEST } from "@/utils/timezoneHelpers";
import { useIsMobile } from "@/hooks/use-mobile";
import { TimezoneToggle, type TzMode } from "@/components/calendar/TimezoneToggle";
...
export default function Calendar() {
  ...
  const [tzMode, setTzMode] = useState<TzMode>(() => {
    return (localStorage.getItem('calendar-tz-mode') as TzMode) || 'local';
  });
  const handleTzModeChange = (mode: TzMode) => {
    setTzMode(mode);
    localStorage.setItem('calendar-tz-mode', mode);
  };
  ...
  const groupedCalls = useMemo(() => {
    ...
    upcomingCalls.forEach(call => {
      const dateKey = getDateFn(call.call_date, call.call_time, call.timezone);
      if (!groups[dateKey]) groups[dateKey] = [];
      groups[dateKey].push(call);
    });
    return Object.entries(groups).sort(([a], [b]) => a.localeCompare(b));
  }, [upcomingCalls, getDateFn]);
  ...
  return <div className="flex-1 overflow-y-auto bg-content" data-tour="calendar-view">
      ...
      <GradientSection className="px-4 md:px-6 py-5" paddingBottom="0px">
        ...
        <div className="flex flex-col items-center text-center mb-6 relative">
          ...
          <div className="mt-3">
            <TimezoneToggle value={tzMode} onChange={handleTzModeChange} />
          </div>
              ...
              {callsLoading ? <p className="text-muted-foreground text-center py-8 text-sm">Loading...</p> : groupedCalls.length === 0 ? <p className="text-muted-foreground text-center py-8 text-sm">No upcoming calls</p> : <div className="space-y-6">
                  ...
                  height: "calc(100% + 1.5rem)"
                      ...
                      <div className="mt-4 ml-6 space-y-3">
                        {dateCalls.map(call => <CallCardRedesigned key={call.id} call={call} isAdmin={canManageCalendar} userTierId={tierId} upsellUrl={upsellUrl} tzMode={tzMode} onEdit={call => {
                    setSelectedCall(call);
