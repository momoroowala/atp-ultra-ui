...
const CourseCatalog = () => {
  ...
  useEffect(() => {
    if (!coursesLoading && courses) {
      const accessibleCourses = courses.filter((c: any) => c.hasAccess);
      if (accessibleCourses.length === 1) {
        navigate(`/courses/${accessibleCourses[0].id}`, { replace: true });
      }
    }
  }, [courses, coursesLoading, navigate]);
            ...
            {courses?.map((course: any) => {
              ...
              return (
                ...
                >
                  {/* Thumbnail */}
                  <div className={`
                    h-16 w-16 md:h-20 md:w-20 rounded-xl shrink-0 flex items-center justify-center
                    ${hasAccess
                      ? "bg-primary/10"
                      : "bg-muted"}
                  `}>
                    {course.thumbnail_url ? (
                      ...
                      <BookOpen className={`h-7 w-7 md:h-8 md:w-8 ${hasAccess ? "text-primary" : "text-muted-foreground"}`} />
                    )}
                  </div>
