...
// === EST-specific helpers ===

const EST_TZ = 'America/New_York';

export const convertToEST = (
  dateStr: string,
  timeStr: string,
  storedTz: string
): Date => {
  ...
  const dateInStoredTz = new Date(year, month - 1, day, hours, minutes);
  const zonedDate = fromZonedTime(dateInStoredTz, storedTz);
  return toZonedTime(zonedDate, EST_TZ);
};

export const formatTimeInEST = (
  dateStr: string,
  timeStr: string,
  storedTz: string
): string => {
  const estDate = convertToEST(dateStr, timeStr, storedTz);
  return format(estDate, 'h:mm a zzz', { timeZone: EST_TZ });
};

export const getDateInEST = (
  dateStr: string,
  timeStr: string,
  storedTz: string
): string => {
...
export const formatDateLabelInEST = (
  dateStr: string,
  timeStr: string,
  storedTz: string
): string => {
  const estDate = convertToEST(dateStr, timeStr, storedTz);
  if (isToday(estDate)) return 'Today';
  if (isTomorrow(estDate)) return 'Tomorrow';
  return format(estDate, 'MMM d');
};

export const isUserInEST = (): boolean => {
  return getUserTimezone() === EST_TZ;
};
