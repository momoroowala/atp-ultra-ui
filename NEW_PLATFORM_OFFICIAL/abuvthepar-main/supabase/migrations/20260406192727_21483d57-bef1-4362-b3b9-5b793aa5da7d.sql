...
-- 2. Seed with current last_sign_in_at as baseline
UPDATE public.user_profiles up
SET last_active_at = au.last_sign_in_at
FROM auth.users au
WHERE au.id = up.id AND up.last_active_at IS NULL;

-- 3. Replace get_users_with_progress to use COALESCE(up.last_active_at, au.last_sign_in_at)
...
BEGIN
  ...
  base AS (
    SELECT up.id, COALESCE(up.user_email, '')::text AS email, up.first_name, up.last_name,
      ...
      COALESCE(up.last_active_at, au.last_sign_in_at) AS last_sign_in_at,
    ...
    AND (p_invitation_filter = 'all'
      OR (p_invitation_filter = 'confirmed' AND COALESCE(up.last_active_at, au.last_sign_in_at) IS NOT NULL)
      OR (p_invitation_filter = 'pending' AND COALESCE(up.last_active_at, au.last_sign_in_at) IS NULL)
      OR (p_invitation_filter = 'logged_in' AND COALESCE(up.last_active_at, au.last_sign_in_at) IS NOT NULL AND up.is_active = true AND COALESCE(up.last_active_at, au.last_sign_in_at) > now() - interval '14 days')
      OR (p_invitation_filter = 'never_logged_in' AND au.last_sign_in_at IS NULL)
      OR (p_invitation_filter = 'at_risk' AND COALESCE(up.last_active_at, au.last_sign_in_at) IS NOT NULL AND up.is_active = true AND COALESCE(up.last_active_at, au.last_sign_in_at) <= now() - interval '14 days')
  ...
  ORDER BY
    ...
    CASE WHEN p_sort_column = 'revenue' AND v_sort_dir = 'asc' THEN base.revenue END ASC NULLS LAST,
    CASE WHEN p_sort_column = 'revenue' AND v_sort_dir = 'desc' THEN base.revenue END DESC NULLS LAST,
