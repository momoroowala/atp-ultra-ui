...
-- Drop policy if exists and recreate (PostgreSQL doesn't have IF NOT EXISTS for policies)
DROP POLICY IF EXISTS "public read" ON public.micro_lessons;
CREATE POLICY "public read" ON public.micro_lessons FOR SELECT USING (true);

-- 2 · Make archetype+lesson_text unique
DO $$ 
BEGIN
    ALTER TABLE public.micro_lessons ADD CONSTRAINT micro_lessons_uni UNIQUE (archetype, lesson_text);
EXCEPTION
