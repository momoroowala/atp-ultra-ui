-- Fix ambiguous column reference in get_users_with_progress by fully qualifying columns
...
BEGIN
  ...
  base AS (
    SELECT
      ...
      up.is_active,
      up.created_at,
      COALESCE(sa.store_activated, false) AS store_activated,
      sa.store_url,
      sa.shopify_shop_name,
      ...
      COALESCE(t.display_name, t.tier_name, t.name, '')::text AS tier,
      COALESCE(r.display_name, r.role_key, '')::text AS role,
    ...
    LEFT JOIN public.shopify_activation_status sa ON sa.user_id = up.id
    WHERE
      (p_search = '' OR (
      ...
      AND (
        p_store_filter = 'all'
        OR (p_store_filter = 'activated' AND COALESCE(sa.store_activated, false) = true)
        OR (p_store_filter = 'not_activated' AND COALESCE(sa.store_activated, false) = false)
      )
      AND (
...
REVOKE ALL ON FUNCTION public.get_users_with_progress(integer, integer, text, uuid[], uuid, text, uuid[], text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_users_with_progress(integer, integer, text, uuid[], uuid, text, uuid[], text, text) TO authenticated;