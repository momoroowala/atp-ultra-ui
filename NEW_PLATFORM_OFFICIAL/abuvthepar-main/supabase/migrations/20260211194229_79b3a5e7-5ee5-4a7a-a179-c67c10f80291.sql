...
DROP FUNCTION IF EXISTS public.get_users_with_progress(integer,integer,text,uuid[],uuid,text,uuid[],text,text);

CREATE OR REPLACE FUNCTION public.get_users_with_progress(p_page integer DEFAULT 1, p_per_page integer DEFAULT 15, p_search text DEFAULT ''::text, p_tier_ids uuid[] DEFAULT NULL::uuid[], p_role_id uuid DEFAULT NULL::uuid, p_store_filter text DEFAULT 'all'::text, p_course_ids uuid[] DEFAULT NULL::uuid[], p_sort_column text DEFAULT 'created_at'::text, p_sort_direction text DEFAULT 'desc'::text, p_invitation_filter text DEFAULT 'all'::text)
 RETURNS TABLE(id uuid, email text, first_name text, last_name text, tier text, tier_id uuid, role text, role_id uuid, is_active boolean, total_tasks integer, completed_tasks integer, progress_percentage integer, created_at timestamp with time zone, store_activated boolean, store_url text, shopify_shop_name text, last_sign_in_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
...
BEGIN
  ...
  WITH total_active_tasks AS (
    ...
    WHERE COALESCE(tsk.is_active, true) = true
  ),
  task_stats AS (
    SELECT tr.user_id,
  ...
  SELECT
    base.id, base.email, base.first_name, base.last_name,
    base.tier_name AS tier, base.tier_id, base.role_name AS role, base.role_id,
    base.is_active, base.total_tasks, base.completed_tasks, base.progress_percentage,
    base.created_at, base.store_activated, base.store_url, base.shopify_shop_name,
    base.last_sign_in_at, base.total_count
  FROM base
  ORDER BY
    ...
    CASE WHEN p_sort_column = 'role' AND v_sort_dir = 'asc' THEN base.role_name END ASC NULLS LAST,
    CASE WHEN p_sort_column = 'role' AND v_sort_dir = 'desc' THEN base.role_name END DESC NULLS LAST,
    CASE WHEN p_sort_column IN ('store_status', 'storeActivated') AND v_sort_dir = 'asc' THEN base.store_activated END ASC NULLS LAST,
    CASE WHEN p_sort_column IN ('store_status', 'storeActivated') AND v_sort_dir = 'desc' THEN base.store_activated END DESC NULLS LAST,
