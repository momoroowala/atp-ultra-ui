-- Drop both overloads of the function
DROP FUNCTION IF EXISTS public.get_users_with_progress(integer,integer,text,uuid[],uuid,uuid[],text,text,text,text,text);
DROP FUNCTION IF EXISTS public.get_users_with_progress(integer,integer,text,uuid[],uuid,uuid[],text,text,text,text,text,text);

-- Recreate with revenue column
CREATE OR REPLACE FUNCTION public.get_users_with_progress(
  ...
  p_csm_filter text DEFAULT NULL::text,
  p_onboarding_filter text DEFAULT 'all'::text,
  p_guarantee_filter text DEFAULT 'all'::text
)
RETURNS TABLE(
  id uuid, email text, first_name text, last_name text, tier text, tier_id uuid,
  role text, role_id uuid, is_active boolean, total_tasks integer, completed_tasks integer,
  progress_percentage integer, created_at timestamp with time zone, last_sign_in_at timestamp with time zone,
  total_count bigint, assigned_csm_id uuid, onboarding_completed boolean, onboarding_date timestamp with time zone,
  offboarding_date date, guarantee_status text, onboarding_booking_status text, revenue integer
)
LANGUAGE plpgsql
