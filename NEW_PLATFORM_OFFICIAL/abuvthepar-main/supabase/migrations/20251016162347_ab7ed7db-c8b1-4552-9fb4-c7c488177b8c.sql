...
-- 2) Drop the foreign key constraint that blocks normalization
ALTER TABLE user_habit_completions 
  DROP CONSTRAINT IF EXISTS user_habit_completions_habit_task_id_fkey;

-- 3) Create view for server-side joining of habit task info
...
-- 4) Fast RPC to assign and get today's habits with task info
...
-- 5) Fast RPC to get overdue habits with task info
...
-- 6) Fast RPC for habit completion summary (for streaks)
...
-- 7) Normalize historical completions to reference user_custom_habits IDs
DO $$
DECLARE
  updated_count integer;
BEGIN
  WITH matching_habits AS (
    SELECT 
      uhc_inner.id AS completion_id,
      uch.id AS new_habit_id
    FROM user_habit_completions uhc_inner
    JOIN habit_tracker_tasks htt ON htt.id = uhc_inner.habit_task_id
    JOIN user_custom_habits uch ON uch.user_id = uhc_inner.user_id AND uch.task_name = htt.task_name
    WHERE uhc_inner.habit_task_id <> uch.id
  )
  UPDATE user_habit_completions
  SET habit_task_id = matching_habits.new_habit_id
  FROM matching_habits
  WHERE user_habit_completions.id = matching_habits.completion_id;
  
  GET DIAGNOSTICS updated_count = ROW_COUNT;
  RAISE NOTICE 'Normalized % historical habit completion records', updated_count;
END $$;

-- 8) Add trigger to seed default habits when new user profile is created
