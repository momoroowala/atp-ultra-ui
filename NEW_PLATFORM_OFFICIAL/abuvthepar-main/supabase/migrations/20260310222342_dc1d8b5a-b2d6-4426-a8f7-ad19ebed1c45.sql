...
DROP TRIGGER IF EXISTS set_trader_profile_id ON public.trader_profiles;

-- Drop tables (CASCADE to handle any remaining dependencies)
DROP TABLE IF EXISTS public.trader_profiles CASCADE;
DROP TABLE IF EXISTS public.trading_accounts CASCADE;
DROP TABLE IF EXISTS public.micro_lessons CASCADE;

-- Drop orphaned functions
DROP FUNCTION IF EXISTS public.set_trader_profile_id_from_auth();
DROP FUNCTION IF EXISTS public.trigger_award_badges_on_trade_log();

-- Recreate check_and_award_achievement_badges without trade-related cases
