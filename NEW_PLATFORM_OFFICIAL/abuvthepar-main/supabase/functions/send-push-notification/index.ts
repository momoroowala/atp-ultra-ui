...
serve(async (req) => {
  ...
  try {
    ...
    // Send all push notifications in parallel. The previous sequential loop
    // scaled as O(n * 300ms); with Promise.allSettled the total wall time is
    // O(slowest single request), which is the critical fix for fan-out latency.
    const sendOne = async (sub: any): Promise<{ status: 'sent' | 'invalid' | 'error'; endpoint: string }> => {
      try {
    ...
    const results = await Promise.allSettled(subscriptions.map(sendOne));

    let successCount = 0;
    const failedEndpoints: string[] = [];
    for (const r of results) {
      if (r.status !== 'fulfilled') continue;
      if (r.value.status === 'sent') successCount++;
      else if (r.value.status === 'invalid') failedEndpoints.push(r.value.endpoint);
    }
