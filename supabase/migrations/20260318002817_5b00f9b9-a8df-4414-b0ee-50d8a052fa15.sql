...
-- Add guarantee_status column
ALTER TABLE public.user_profiles
  ADD COLUMN IF NOT EXISTS guarantee_status text DEFAULT 'pending';

-- Recreate RPC with new fields
DROP FUNCTION IF EXISTS public.get_users_with_progress;
...
RETURNS TABLE(
  ...
  tier text, tier_id uuid, role text, role_id uuid, is_active boolean,
  total_tasks integer, completed_tasks integer, progress_percentage integer,
  created_at timestamp with time zone, last_sign_in_at timestamp with time zone,
  total_count bigint, assigned_csm_id uuid, onboarding_completed boolean,
  onboarding_date date, offboarding_date date, guarantee_status text
)
LANGUAGE plpgsql
...
BEGIN
  ...
  base AS (
    ...
    AND (p_onboarding_filter = 'all'
      OR (p_onboarding_filter = 'completed' AND COALESCE(up.onboarding_completed, false) = true)
      OR (p_onboarding_filter = 'in_progress' AND COALESCE(up.onboarding_completed, false) = false AND COALESCE(ts.completed_count, 0) > 0)
      OR (p_onboarding_filter = 'not_started' AND COALESCE(up.onboarding_completed, false) = false AND COALESCE(ts.completed_count, 0) = 0)
    )
  )
  SELECT base.id, base.email, base.first_name, base.last_name, base.tier_name, base.tier_id,
    ...
    base.onboarding_date, base.offboarding_date, base.guarantee_status
  FROM base
  ORDER BY
