-- 1. Replace check_and_award_achievement_badges with all requirement types handled
      ...
      WHEN 'early_submission' THEN
        IF v_user_created_at IS NOT NULL AND EXISTS (
          ...
          WHERE user_id = p_user_id AND status = 'completed'
            AND submitted_at <= v_user_created_at + INTERVAL '24 hours'
        ) THEN
          INSERT INTO user_achievement_badges (user_id, badge_id) VALUES (p_user_id, v_badge.id) ON CONFLICT (user_id, badge_id) DO NOTHING;
        END IF;
              ...
              AND NOT EXISTS (
                ...
                WHERE t.phase_id = p.id AND t.is_active = true
                  AND NOT EXISTS (
                    SELECT 1 FROM task_responses tr
                    WHERE tr.task_id = t.id AND tr.user_id = p_user_id AND tr.status = 'completed'
                      AND tr.submitted_at <= v_user_created_at + (v_days || ' days')::INTERVAL
                  )
              )
          ) THEN
            INSERT INTO user_achievement_badges (user_id, badge_id) VALUES (p_user_id, v_badge.id) ON CONFLICT (user_id, badge_id) DO NOTHING;
...
-- 2. Trigger functions for community actions
CREATE OR REPLACE FUNCTION public.trigger_award_badges_on_message()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  PERFORM check_and_award_achievement_badges(NEW.sender_id);
  RETURN NEW;
END;
$function$;

CREATE OR REPLACE FUNCTION public.trigger_award_badges_on_reaction()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- 3. Create triggers
DROP TRIGGER IF EXISTS award_badges_after_message ON public.community_messages;
CREATE TRIGGER award_badges_after_message
  AFTER INSERT ON public.community_messages
  FOR EACH ROW
  EXECUTE FUNCTION trigger_award_badges_on_message();

DROP TRIGGER IF EXISTS award_badges_after_reaction ON public.community_message_reactions;
CREATE TRIGGER award_badges_after_reaction
  AFTER INSERT ON public.community_message_reactions
  FOR EACH ROW
  EXECUTE FUNCTION trigger_award_badges_on_reaction();