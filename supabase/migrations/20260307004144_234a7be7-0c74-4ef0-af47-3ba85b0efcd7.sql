...
-- Drop old overload without csm/onboarding params
DROP FUNCTION IF EXISTS public.get_users_with_progress(integer, integer, text, uuid[], uuid, uuid[], text, text, text);

-- Replace the newer overload with extended invitation_filter logic
...
RETURNS TABLE(
  ...
  assigned_csm_id uuid, onboarding_completed boolean
)
