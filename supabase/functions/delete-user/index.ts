...
serve(async (req) => {
  ...
  try {
    ...
    const deletedBy = authMethod === 'API_KEY' ? 'API_KEY' : authenticatedUserEmail;

    // Step 1: Delete auth user FIRST (this is the critical operation)
    ...
    console.log('[AUDIT] User deleted:', { userId, deletedBy, timestamp: new Date().toISOString() });

    // Step 2: Only clean up table data AFTER auth deletion succeeds
    // If any table cleanup fails, log but continue (orphaned data is better than partial deletion)
    const tablesToClean = [
      'ai_chat_sessions',
    ...
    try {
      ...
      console.error('Error during table cleanup (auth user already deleted):', cleanupError);
    }
