...
interface RecordingCreateModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}


export const RecordingCreateModal = ({ open, onOpenChange }: RecordingCreateModalProps) => {
  const { createRecording } = useCallRecordings();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!date || !title || !recordingUrl) return;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Weekly Coaching Call - Jan 15"
              required
            />
          </div>
