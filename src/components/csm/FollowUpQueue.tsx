...
export function FollowUpQueue({ students, outreachStatuses, onStatusChange, activeFilter, onClearFilter, hideHeader, readOnly }: FollowUpQueueProps) {
  ...
  const PENDING_STATUSES = new Set(['follow_up_required', 'no_action', undefined, '']);
  const contacted = filtered.filter(s => {
    const st = outreachStatuses[`${s.id}_${s.metricType}`];
    return st && !PENDING_STATUSES.has(st);
  }).length;
  ...
  return (
    ...
    >
      ...
      {/* Table Header — Sticky */}
      <div className="hidden md:grid grid-cols-[36px_minmax(180px,1fr)_80px_60px_60px_80px_90px] gap-1 px-3 py-1.5 text-[9px] font-medium text-muted-foreground border-b bg-card sticky top-0 z-10">
        <span></span>
        <span>Student</span>
        <span>Category</span>
        <button
        ...
        {visible.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {visible.map((s) => {
              ...
              const isFollowUp = currentStatus === 'follow_up_required';
              ...
              return (
                <div key={statusKey}>
                  {/* Desktop row */}
                  <div
                    className={`hidden md:grid group grid-cols-[36px_minmax(180px,1fr)_80px_60px_60px_80px_90px] gap-1 items-center px-3 py-1 text-xs transition-all duration-200 ${rowBg}`}
                  >
                    <div
                    ...
                    <div className="min-w-0">
                      <span
                        className="cursor-pointer hover:underline text-foreground font-medium truncate text-[13px] block"
                        onClick={(e) => { e.stopPropagation(); navigate('/csm-panel', { state: { viewStudentId: s.id, activeTab: 'students' } }); }}
                      >
                  ...
                  <div className={`md:hidden flex flex-col gap-1.5 px-3 py-2 text-xs transition-all duration-200 ${rowBg}`}>
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <span
                          className="cursor-pointer hover:underline text-foreground font-medium truncate text-[12px] block"
                          onClick={(e) => { e.stopPropagation(); navigate('/csm-panel', { state: { viewStudentId: s.id, activeTab: 'students' } }); }}
                        >
                          {s.firstName || s.lastName ? `${s.firstName} ${s.lastName}`.trim() : s.email}
                        </span>
