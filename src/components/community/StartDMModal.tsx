...
export const StartDMModal = ({ open, onOpenChange }: StartDMModalProps) => {
  ...
  const { createOrGetDM: _rawCreateDM, createOrGetDMWithFallback, createGroupDM } = useCommunityDMs();
    ...
    const searchUsers = async () => {
      ...
      try {
        ...
        const adminRoleIds = (roles || [])
          .filter(r => ['admin', 'csm', 'executive'].includes(r.role_key))
          .map(r => r.id);
        ...
        // Staff can see all users; clients only see non-staff
        // Staff can see everyone; clients can only see staff members
        const filteredUsers = isStaff
          ? (data || [])
          : (data || []).filter(u => adminRoleIds.includes(u.role_id as string));
  ...
  const handleStartConversation = () => {
    ...
    if (selectedUsers.length === 1) {
      ...
      createOrGetDMWithFallback.mutate(
        { userId: u.id, firstName: u.first_name || undefined, lastName: u.last_name || undefined, email: u.user_email || undefined },
        {
          onSuccess: (result) => {
            if (result) {
              onOpenChange(false);
