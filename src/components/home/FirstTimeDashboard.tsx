...
import { Play, CheckCircle2, Map, Sparkles, Compass } from "lucide-react";
...
export const FirstTimeDashboard = ({
  ...
  return (
    <SpotlightBackground className="flex-1 flex flex-col min-w-0 overflow-y-auto bg-background">
      ...
      <div className="px-4 md:px-8 pb-6 relative z-10">
        <div className="max-w-4xl mx-auto">
          <button
            ...
            className="w-full text-center py-3 px-4 rounded-xl border border-primary/20 bg-primary/5 text-primary text-sm font-medium hover:bg-primary/10 transition-colors"
          >
            ✅ Already finished your first lesson? Click here to unlock your full Dashboard
          </button>
        </div>
      </div>
      ...
      <button
        onClick={() => {
          localStorage.removeItem("onboarding_video_watched");
          localStorage.removeItem("onboarding_tour_completed");
          window.location.reload();
        }}
        className="fixed bottom-4 right-4 z-50 flex items-center gap-1.5 rounded-full bg-muted/80 backdrop-blur px-3 py-1.5 text-xs text-muted-foreground opacity-40 hover:opacity-100 transition-opacity border border-border"
      >
